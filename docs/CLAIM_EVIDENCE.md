...
### C-16 production 扩展包在真实 Chrome 里真人复验通过
...
- **证据细节**：可见性由布局量测决定（放不下就退成 40px 贴边 dock，**不再整块消失**）；
  ...
  各有一个 PoC 验证过会失败
- **必须同时说的限定**：**dock 与停靠偏好都还没有真人复验**（U-18 / U-19）。
...
| **U-19** | **停靠偏好的真人复验** | L1/L2 | 默认改成 left、不再自动跳边。单测 + 浏览器里验过（左 12–236 / 右 1204–1428 / 切回左），但没有真人在真实知乎上来回切页看过。和 U-18 一起等一次完整真人验收 |
...
| ~~U-14~~ | ~~production 扩展包在真实 Chrome 里装上之后的表现~~ | **已升级 L4 · CLOSED** | 2026-09-14 真人复验 PASS：真实回答页 → `semantic/high` → 364 chars → 8s qualified → Dashboard visible → 徽标「**AI 分析**」→ 1/10 入库。已升级为 SUPPORTED claim **C-16** |
