...
import { buildNeighborhood, DEFAULT_GROUNDING_CONFIG } from './grounding.ts';
import { l2Normalize, termFrequency } from './tokenize.ts';
import type { GroundingConfig } from './grounding.ts';
import type { ZhihuSearchClient } from './zhihu-client.ts';
import type { CandidateConcept, SemanticNeighborhood } from './types.ts';

export class ConceptCorpus {
  #client: ZhihuSearchClient;
  #config: GroundingConfig;
  #size: number;
  #neighborhoods: Map<string, SemanticNeighborhood> | null = null;
  /** 跨全部概念的文档频率，用于召回阶段的粗粒度判别 */
  #globalDf: Map<string, number> = new Map();
  #failures: string[] = [];

  constructor(client: ZhihuSearchClient, size = 8, config: GroundingConfig = DEFAULT_GROUNDING_CONFIG) {
    this.#client = client;
    this.#config = config;
    this.#size = size;
  }
  ...
  async ensureLoaded(): Promise<Map<string, SemanticNeighborhood>> {
    if (this.#neighborhoods) return this.#neighborhoods;

    const map = new Map<string, SemanticNeighborhood>();
  ...
  neighborhoodsFor(labels: string[]): SemanticNeighborhood[] {
    const out: SemanticNeighborhood[] = [];
   ...
   */
  async recall(title: string, text: string, k = 5): Promise<CandidateConcept[]> {
    ...
    const scored: Array<{ label: string; score: number; terms: Array<[string, number]> }> = [];
