...
import type { ZhihuSearchItem } from './types.ts';

export type EvidenceLevel =
  /** 不提供任何知乎证据 —— 即 LLM-only 对照组 */
  | 'none'
  /** 只给标题 */
  | 'titles'
  /** 标题 + 内容摘要 */
  | 'summaries'
  /** 标题 + 摘要 + 精选评论 + 官方排序/权威/互动信号 */
  | 'full';
...
export interface CandidateEvidence {
  label: string;
  query: string;
  /** 实际取回的条数。0 表示知乎侧没有语料，裁决时必须知道这一点 */
  itemCount: number;
  items: EvidenceItem[];
  /** 该候选取证是否失败（与"取回 0 条"是两回事） */
  failed?: boolean;
}
...
function project(it: ZhihuSearchItem, level: EvidenceLevel): EvidenceItem {
  const base: EvidenceItem = { title: it.Title };
 ...
 */
export function renderEvidence(bundles: CandidateEvidence[], level: EvidenceLevel): string {
