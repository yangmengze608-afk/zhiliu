 ...
 */
import { buildPrompt, OllamaExtractor, PROMPT_VERSION } from './llm.ts';
import type { LLMExtractor, LlmExtractionResult } from './llm.ts';
import { validateExtraction, MAX_CONCEPTS } from './extract.ts';
import type { ExtractionInput } from './extract.ts';
import { ANALYSIS_VERSION } from './model.ts';

export type ExtractorProviderName = 'ollama' | 'openai_compatible' | 'zhida';

export interface ProviderConfig {
  provider: ExtractorProviderName;
  model?: string;
  baseUrl?: string;
  /** 只允许来自服务端环境变量。绝不进入扩展包。 */
  apiKey?: string;
  timeoutMs?: number;
...
export class OpenAICompatibleExtractor implements LLMExtractor {
  ...
  constructor(cfg: ProviderConfig & {
    ...
    /**
     * **函数，不是对象。** 第一版写成了静态对象，于是 `X-Request-Timestamp`
     * 在构造函数里算一次就固定了——服务端校验这个时间戳，进程跑过几分钟之后
     * 每一次请求都会带着一个过期的时间戳。单测发现不了，因为单测的 fetch 是假的。
     */
    extraHeaders?: () => Record<string, string>;
    maxAttempts?: number;
    backoffBaseMs?: number;
  ...
  get rateLimitState(): { open: boolean; consecutive429: number; retryAfterMs: number } {
    ...
    try {
      for (let attempt = 1; attempt <= this.#maxAttempts; attempt++) {
        ...
        if (!resp.ok) {
          ...
          let code = '';
          try {
            const e = (await resp.json()) as { error?: { code?: string } };
...
export class ZhidaExtractor extends OpenAICompatibleExtractor {
  constructor(cfg: Omit<ProviderConfig, 'provider'> & {
    accessSecret: string; maxAttempts?: number; backoffBaseMs?: number; maxBackoffMs?: number;
  }) {
...
export async function probeZhida(
  accessSecret: string,
  model: string = ZHIDA_MODELS[0],
  fetchImpl: typeof fetch = fetch,
): Promise<{ ok: boolean; httpStatus: number; shapeMatchesOpenAI: boolean; rawPreview: string }> {
 ...
 */
export function createExtractor(cfg: ProviderConfig & { accessSecret?: string }): LLMExtractor {
  switch (cfg.provider) {
    case 'ollama':
 ...
 * "直答输出和本地模型一样规整"的证据。
 */
export function safeParse(s: string): { unknown?: boolean; [k: string]: unknown } | null {
...
function tryJson(t: string): { unknown?: boolean; [k: string]: unknown } | null {
  try {
