...
export interface SearchTransport {
  search(query: string, count: number): Promise<ZhihuSearchResponse>;
...
export class LiveSearchTransport implements SearchTransport {
  kind: 'live' = 'live';
  #secret: string;
  #fetch: typeof fetch;
  #calls = 0;

  constructor(accessSecret: string, fetchImpl: typeof fetch = fetch) {
  ...
  async search(query: string, count: number): Promise<ZhihuSearchResponse> {
    ...
    const body = (await resp.json()) as ZhihuSearchResponse;
    if (body.Code !== 0) {
...
export class FixtureSearchTransport implements SearchTransport {
  ...
  constructor(fixtures: Record<string, ZhihuSearchResponse>) {
...
export class ZhihuSearchClient {
  ...
  #ttlMs: number;
  #now: () => number;
  #cache = new Map<string, { at: number; value: ZhihuSearchItem[] }>();

  constructor(transport: SearchTransport, ttlMs = 24 * 60 * 60 * 1000, now: () => number = Date.now) {
    this.#transport = transport;
    this.#ttlMs = ttlMs;
    this.#now = now;
  }

  get transportKind(): 'live' | 'fixture' {
  ...
  async neighborhoodItems(query: string, count = 8): Promise<ZhihuSearchItem[]> {
