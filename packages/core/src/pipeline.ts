...
import { DEFAULT_GROUNDING_CONFIG, buildNeighborhood, groundCandidates } from './grounding.ts';
import { DEFAULT_ADJUDICATION_CONFIG, adjudicate } from './adjudicate.ts';
import type { GroundingConfig } from './grounding.ts';
import type { AdjudicationConfig } from './adjudicate.ts';
import { ConceptCorpus } from './corpus.ts';
import { CONCEPTS } from './registry.ts';
import type { ZhihuSearchClient } from './zhihu-client.ts';
import type { AnalysisResult, CandidateConcept, ExtractedContent } from './types.ts';

export interface AnalyzeOptions {
  /**
   * 候选召回方式。
   * - `corpus`（默认）：用知乎语义邻域做召回，系统路径上完全不依赖手写线索词
   * - `cues`：用 concepts.json 里的手写线索词召回（消融实验用，也是 Baseline A 的做法）
   */
  recall?: 'corpus' | 'cues';
  /** 复用同一个语料库实例，避免每篇重建（生产必须复用） */
 ...
 */
export function analyzeLexicalOnly(content: ExtractedContent, options: AnalyzeOptions = {}): AnalysisResult {
...
export async function analyzeGivenCandidates(
  ...
  const candidates: CandidateConcept[] = candidateLabels.map((label, i) => ({
    label,
    query: label,
