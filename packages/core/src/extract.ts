 ...
 */
import { canonicalizeAll } from './canonicalize.ts';
import { ANALYSIS_VERSION } from './model.ts';
import type { AnalysisStatusV2 } from './model.ts';
...
export interface RawExtraction {
  concepts?: Array<{ canonical?: string; label?: string; confidence?: number }>;
  ambiguous?: boolean;
  unknown?: boolean;
}
...
export interface ExtractionResult {
  ...
  /** 被丢弃或无法归一的标签，保留用于诊断概念表覆盖缺口 */
  dropped: string[];
  /** 被修复过的字段（如模型漏给 confidence）。用于审计"我们替模型补了什么" */
  repaired?: string[];
}
 ...
 */
export function validateExtraction(raw: unknown, source: AnalysisStatusV2 = 'ungrounded'): ExtractionResult {
  const fail = (why: string): ExtractionResult => ({
...
export function countsTowardStats(r: ExtractionResult): boolean {
  return r.status !== 'failed' && !r.ambiguous && r.concepts.length > 0;
}

export interface ConceptExtractor {
  readonly kind: AnalysisStatusV2;
