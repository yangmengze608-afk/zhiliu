...
import { validateExtraction } from '../../../packages/core/src/extract.ts';
import type { ExtractionResult } from '../../../packages/core/src/extract.ts';
import { ANALYSIS_VERSION } from '../../../packages/core/src/model.ts';
import type { AnalysisMeta, AnalysisStatusV2 } from '../../../packages/core/src/model.ts';

/** 未接模型时使用的出处标记。写进历史，将来一眼能认出这段数据不是模型产的。 */
const MOCK_META: AnalysisMeta = {
  version: ANALYSIS_VERSION, promptVersion: 'n/a', provider: 'local-mock', model: 'deterministic-hash',
};

/** 失败记录也要带出处，否则历史里会出现"不知道是谁失败了"的空洞。 */
function failed(why: string, meta: AnalysisMeta = { version: ANALYSIS_VERSION, promptVersion: 'n/a' }): AnalysisOutcome {
...
/** 只允许出现在网络请求里的字段。 */
const WIRE_FIELDS = ['contentId', 'contentType', 'title', 'text'] as const;

export function toWirePayload(req: AnalyzeRequest): Record<string, unknown> {
...
const MOCK_SETS: Array<Array<{ canonical: string; confidence: number }>> = [
...
export type AnalysisOutcome = ExtractionResult & {
  /** 服务端声明的模型来源。缺失即无法证明是模型产出 */
  provenance?: { provider?: string; model?: string; promptVersion?: string; latencyMs?: number };
  /** 写进 ReadEvent 的出处。mock 也有，只是 provider='local-mock' */
  analysis: AnalysisMeta;
};

export async function requestAnalysis(req: AnalyzeRequest): Promise<AnalysisOutcome> {
  // **只有端点为空时才走 mock。**
  //
...
export function mockAnalysis(req: AnalyzeRequest): AnalysisOutcome {
