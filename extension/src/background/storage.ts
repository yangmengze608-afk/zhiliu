 ...
 */
import { CONFIG, STORAGE_KEYS, DEFAULT_SETTINGS } from '../shared/config.ts';
import type { Settings } from '../shared/config.ts';
import type { AnalysisMeta, ReadEvent } from '../../../packages/core/src/model.ts';
...
function area(): StorageArea {
  return (globalThis as unknown as { chrome: { storage: { local: StorageArea } } }).chrome.storage.local;
}
 ...
 */
export async function loadEvents(): Promise<ReadEvent[]> {
...
export function normalizeEvent(raw: Record<string, unknown>): ReadEvent {
  const concepts = Array.isArray(raw.concepts)
    ? (raw.concepts as unknown[]).filter(isObject).map((c) => ({
  ...
  return {
    ...
    concepts,
    analysisStatus: typeof raw.analysisStatus === 'string' ? (raw.analysisStatus as ReadEvent['analysisStatus']) : 'failed',
...
function normalizeMeta(raw: unknown): AnalysisMeta | undefined {
...
export async function appendEvent(e: ReadEvent): Promise<ReadEvent[]> {
...
export async function replaceEvents(events: ReadEvent[]): Promise<void> {
  await area().set({ [STORAGE_KEYS.EVENTS]: trim(events) });
}

export function trim(events: ReadEvent[], max = CONFIG.MAX_RECORDS): ReadEvent[] {
  return events.slice(-max);
}

export async function loadSettings(): Promise<Settings> {
  const raw = await area().get(STORAGE_KEYS.SETTINGS);
  return { ...DEFAULT_SETTINGS, ...((raw[STORAGE_KEYS.SETTINGS] as Partial<Settings>) ?? {}) };
}
