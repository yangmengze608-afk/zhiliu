...
import { extractContent, isAnalyzable } from './extract-content.ts';
...
import type { PanelView } from './dashboard.ts';
import { CONFIG, DEFAULT_SETTINGS } from '../shared/config.ts';
import type { Settings } from '../shared/config.ts';
import { DEMO_STEP_MS } from '../shared/demo-data.ts';
...
async function boot(): Promise<void> {
  ...
  installDiagnostics(() => ({
    ...
    view: () => send<PanelView>({ type: 'GET_VIEW' }),
  }));
  await refresh();
