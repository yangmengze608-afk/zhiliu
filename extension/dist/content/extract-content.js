...
import { blockText, sanitize, stripNoise, textWithoutNoise, truncateForModel } from './sanitize.js';
 ...
 */
export const SUPPORTED_TYPES = ['answer', 'article']         ;
 ...
 */
export function extractContent(doc           = document, href         )                   {
  ...
  if (!(SUPPORTED_TYPES                     ).includes(id.type)) {
 ...
 */
export function locateContentElement(doc           = document, href         )                 {
  const id = href === undefined ? detectPage() : detectPage(href);
  if (!(SUPPORTED_TYPES                     ).includes(id.type)) return null;
  const kind = id.type                        ;
 ...
 */
export function largestTextBlock(root         , title = '')         {
  ...
  const candidates            = [clean, ...clean.querySelectorAll('div, section, article, p')];
 ...
 */
export function isAnalyzable(c                  )          {
  return (SUPPORTED_TYPES                     ).includes(c.type)
...
export function pickRoots(
  ...
  const order = (g                 ) => [...g].sort((a, b) => b.depth - a.depth);
 ...
 */
function groupByContainment(cands                 )                    {
  const groups                    = [];
 ...
 */
function foreignIdNear(el         , target        , benign                   )                {
  const known = new Set        ([target, ...benign]);
