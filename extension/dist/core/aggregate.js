 ...
 * 全部在浏览器本地计算，不上传。
 */
import { concentration, buildDistribution, MIN_SAMPLES, WINDOW_SIZE } from './concentration.js';
 ...
 */
function prevalenceIn(events             , canonical        )         {
  if (events.length === 0) return 0;
  return countIn(events, canonical) / events.length;
}

/** 出现篇数。趋势判定用整数比较，避免浮点误差。 */
function countIn(events             , canonical        )         {
...
export function buildAggregates(all             , windowSize = WINDOW_SIZE)                {
  ...
  const concepts                     = [...byConcept.entries()].map(([canonical, v]) => {
  ...
  return {
    ...
    analysisMix: [...stat.reduce((m, e) => m.set(analysisKey(e), (m.get(analysisKey(e)) ?? 0) + 1), new Map                ())]
...
export function trendDirection(
  recentN        , priorN        , recentCount        , priorCount        ,
)                         {
