 ...
 * 区分性词汇也就不同。
 */
import { dot, l2Normalize, termFrequency } from './tokenize.js';
import { surfaceFormsOf } from './registry.js';
...
export function buildNeighborhood(
  ...
  query        ,
  items                   ,
  config                  = DEFAULT_GROUNDING_CONFIG,
)                       {
 ...
 * 它只影响解释文案，不影响任何得分。
 */
export function pickExplanationTerms(contrib                         , limit = 8)           {
  const kept           = [];
